...
const joke_text1 = document.getElementById("joke-text1");
const button = document.getElementById("button");

async function jokeTeller(v) {
  const res = await fetch("https://backend-omega-seven.vercel.app/api/getjoke");
  const data = await res.json();
  console.log(data);
  //   console.log(data[0].question);
  joke_text.textContent = data[0].question;
  joke_text1.textContent = data[0].punchline;
}
